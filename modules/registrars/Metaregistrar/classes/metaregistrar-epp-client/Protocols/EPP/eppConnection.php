...
class eppConnection {
    ...
    */
    ...
    /**
     *
     * @var boolean $whmcsLogging
     */
    protected $whmcsLogging;
    //-------------------------------------------------------------------------- 
    /**
        ...
        if ($this->write($content->saveXML(null, LIBXML_NOEMPTYTAG))) {
            ...
            if (strlen($xml)) {
                ...
                if ($response->loadXML($xml)) {
                    ...
                    $this->writeLog($response->saveXML(null, LIBXML_NOEMPTYTAG), "READ");
                    //WHMCS MODIFICATION----------------------------------------
                    $this->writeWhmcsLog($content ,$response);
                    //----------------------------------------------------------
                    $clienttransid = $response->getClientTransactionId();
    ...
    //WHMCS MODIFICATION--------------------------------------------------------
    public function enableWhmcsLogging() {
        $this->whmcsLogging = true;
    }
    //--------------------------------------------------------------------------
    ...
    //WHMCS MODIFICATION--------------------------------------------------------
    protected function writeWhmcsLog($request, $response) {
        if ($this->whmcsLogging && function_exists("logModuleCall")) {
            $request->formatOutput = true;
            $reqestClass    = get_class($request);
            $reqestString   = $request->saveXML();
